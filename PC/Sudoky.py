...
class SudokuGUI:
        ...
        while self.ser and self.ser.is_open:
            try:
                if self.ser.in_waiting > 0:
                    ...
                    buffer.extend(chunk)
                    print(f"[RX RAW CHUNK]: {chunk.hex(' ').upper()}")  # Вивід кожного байта
                    ...
                    while len(buffer) >= 6:
                        ...
                        if cmd_type in [CMD_START, CMD_RESTART,CMD_CHEAT]:
                            ...
                            if calc_crc == packet[83]:
                                ...
                                print(f"   \033[91m[CRC ERROR]\033[0m Got {hex(packet[83])}, expected {hex(calc_crc)}")
                                print("=============================")
                            ...
                            if calc_crc == packet[5]:
                                ...
                                if cmd_type == CMD_SET:
                                    if status == 0x10:
                                        ...
                                        b2 = packet[3]
                                        b3 = packet[4]
                                        # Оновлюємо конкретну клітинку в GUI
                                        self.root.after(0, self.update_single_cell, b1, b2, b3)
                                    if status == 0x11:
                                        b1 = packet[2]
                                        b2 = packet[3]

                                        # Виділяємо клітинку зеленим і повідомляємо про заблокованість клітинки
                                ...
                                if cmd_type == 0x05:
                                    ...
                                    if status == 0x10:  # STATUS_OK
                                        # Викликаємо функцію очищення
                                        self.root.after(0, self.clear, b1, b2)
                                    elif status == 0x12:  # STATUS_LOCKED
                                        # Якщо клітинка заблокована (умова задачі), підсвічуємо її
                                        self.root.after(0, self.locked_cell, b1, b2, 0)
                                if cmd_type == CMD_FIELD:
                                    total = packet[2]  # Байт 2: скільки було нулів
                                    current = packet[3]  # Байт 3: скільки зараз нулів
                                    self.root.after(0, self.refresh_progress, total, current)
                                if cmd_type == CMD_GIVEUP:
                                    if status == 0x14:
                                        print("   \033[93m[GIVE UP]\033[0m Гравець здався")
                                ...
                                if cmd_type == CMD_HELP:
                                    ...
                                    else:self.root.after(0, self.locked_cell, b1, b2, 0)
                                if cmd_type == CMD_DIFFICULTY:
                                    if status == 0x16:  # STATUS_SETDIF
                                        level = packet[2]  # Отримуємо рівень з байта b1
                                        print(f"   [SYSTEM] Складність {level} підтверджена! Розблоковую START.")
    ...
    def apply_hint_result(self, r, c, val):
        """Оновлює клітинку даними підказки від STM32"""
        if 0 <= r < 9 and 0 <= c < 9:
            # 1. Встановлюємо текст (цифру)
            display_text = str(val) if val != 0 else ""

            # 2. Фарбуємо у фіолетовий колір (колір магії/підказки)
            self.cells[r][c].config(text=display_text, fg="#8e44ad")

            # 3. Додаємо ефект "спалаху" (жовтий фон на 300 мс), щоб гравець помітив, де з'явилася цифра
            original_bg = self.cells[r][c].cget("bg")
            self.cells[r][c].config(bg="#fff9c4")  # Світло-жовтий
    ...
    def apply_difficulty_confirmed(self, level):
        """Викликається після відповіді 0x16 від STM32"""
        # Словник кольорів для кожного рівня
        colors = {
            1: "#2ecc71",  # Яскраво-зелений для 🙂
            2: "#f1c40f",  # Жовтий/Золотий для 😐
            3: "#e74c3c"  # Червоний для 😡
        }

        for i, btn in enumerate(self.diff_buttons, 1):
            if i == level:
                # Обраний смайл стає великим і кольоровим
                btn.config(font=("Arial", 35, "bold"), fg=colors.get(level, "black"))
            else:
                # Інші залишаються маленькими і стають напівпрозорими (сірими)
    ...
    def clear(self, r, c):
        """Очищує вміст клітинки в інтерфейсі"""
        self.cells[r][c].config(text="")

    def give_up(self):
        """Обробка підтвердження поразки: повернення в меню"""
        messagebox.showinfo("Game Over", "Ви здалися! Повертаємось до головного меню.")

        # 1. Видаляємо ігровий інтерфейс
        if hasattr(self, 'main_ui'):
            self.main_ui.destroy()

        # 2. Скидаємо стан гри в початкове положення
        self.game_started = False
        self.initial_field = None
        self.progress_var.set("Прогрес: 0%")

        # 3. Малюємо початкове меню (порт, кнопка підключення тощо)
        self.create_menu()



    def invalid(self, r, c):
        """Блимає клітинкою, якщо хід порушує правила Судоку"""
        # Жовтий колір для "Увага, так не можна за правилами"
    ...
    def locked_cell(self, r, c, val):
        """Підсвічує клітинку, яку не можна змінювати (LOCKED)"""
        if 0 <= r < 9 and 0 <= c < 9:
            # Зберігаємо поточний колір, щоб потім його повернути
            original_color = self.cells[r][c].cget("bg")

            # Підсвічуємо червоним або помаранчевим (колір попередження)
            # Ти просив зелений, але зазвичай для помилок використовують теплі кольори
            self.cells[r][c].config(bg="#ff7675")  # Ніжно-червоний

            # Через 500 мс повертаємо колір назад
            self.root.after(500, lambda: self.cells[r][c].config(bg=original_color))

            # Можна також додати короткий звук або оновити статус
            print(f"Клітинка ({r}, {c}) заблокована!")
    ...
    def mega_win(self):
        """Створює святкове вікно перемоги"""
        # Створюємо нове вікно поверх головного
        win_window = tk.Toplevel(self.root)
        win_window.title("ПЕРЕМОГА! 🎉")
        win_window.geometry("400x250")
        win_window.configure(bg="#f1f2f6")

        # Робимо вікно модальним (не можна клікати на основне поле, поки не закриєш це)
        ...
        tk.Label(
            win_window,
            text="Ви успішно розв'язали Судоку!\nВсі цифри на своїх місцях.",
            font=("Arial", 12),
            bg="#f1f2f6"
        ).pack(pady=10)
        ...
        re_btn = tk.Button(
            ...
            fg="white",
            width=15,
            command=lambda: [win_window.destroy(), self.send_cmd(0x02)]  # 0x02 - CMD_RESTART
        )
        re_btn.pack(side=tk.LEFT, padx=10)
    ...
    def create_menu(self):
        ...
        # 1. Заголовок
        tk.Label(self.menu_frame, text="SUDOKU STM32", font=("Arial", 30, "bold")).pack(pady=(50, 20))

        # 2. Кнопка START (заблокована, поки не обрано рівень)
        ...
        # 3. Блок складності
        tk.Label(self.menu_frame, text="Оберіть рівень складності:", font=("Arial", 10)).pack(pady=(10, 5))
        diff_frame = tk.Frame(self.menu_frame)
        diff_frame.pack(pady=5)

        self.diff_buttons = []
        levels = [("🙂", 1), ("😐", 2), ("😡", 3)]

        # Стан кнопок залежить від того, чи відкритий порт
        ...
        for emoji, level in levels:
            btn = tk.Button(
                ...
                cursor="hand2",
                state=init_state,
                command=lambda l=level: self.select_difficulty_request(l)
            )
            btn.pack(side=tk.LEFT, padx=10)
            self.diff_buttons.append(btn)

        # Пружина для вирівнювання
        spacer = tk.Frame(self.menu_frame)
        spacer.pack(expand=True)

        # 4. Блок підключення
        tk.Label(self.menu_frame, text="Налаштування зв'язку:", font=("Arial", 10)).pack(pady=(10, 0))
    ...
    def create_game_ui(self):
        ...
        side = tk.Frame(self.main_ui)
        side.pack(side=tk.LEFT, padx=20)

        tk.Label(side, textvariable=self.progress_var, font=("Arial", 12, "bold")).pack()
        self.progressbar = ttk.Progressbar(side, length=150, mode='determinate')
        self.progressbar.pack(pady=10)

        for i in range(1, 10):
            if (i - 1) % 3 == 0: f = tk.Frame(side); f.pack()
            tk.Button(f, text=str(i), width=4, height=2, command=lambda n=i: self.set_val(n)).pack(side=tk.LEFT, padx=2,
                                                                                                   pady=2)
        ...
        tk.Button(side,text="GIVE UP",bg="#e67e22",fg="white",font=("Arial", 10, "bold"),command=self.give_up_action,width=15).pack(pady=(40, 10))

        right_panel = tk.Frame(self.main_ui, bg="")
        ...
        self.hint_btn = tk.Button(
            ...
            bd=0,
            cursor="hand2",
            command=self.noob_help

        )
        self.hint_btn.pack(pady=(0, 10))

        grid_container = tk.Frame(right_panel)
    ...
    def select_difficulty_request(self, level):
        """Надсилає запит на встановлення складності (0x08)"""
        # Відправляємо команду 0x08, рівень у b1
